...
export default async function PropertPage({ params }: PageProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-1 py-10 lg:px-4 lg:py-14">
      <Link
        href="/biblioteca/artigos"
        className="mb-6 ml-2 inline-flex items-center gap-2 rounded-2xl border border-slate-200 px-3 py-1.5 text-sm font-semibold text-slate-600 transition hover:text-orange-600"
      >
          ...
          <Image
            src={
              artigo.capa && artigo.capa.trim()
                ? artigo.capa
                : "https://placehold.co/1200x720?text=Artigo"
            }
            alt={artigo.titulo}
            fill
        ...
        <div className="space-y-6">
          <div className="p-6 md:p-10">
            <div className="space-y-3">
              <h1 className="text-xl font-bold text-slate-900 sm:text-2xl md:text-4xl lg:text-3xl">
                {artigo.titulo}
              </h1>
              <p className="pt-3 text-lg text-slate-700">{artigo.resumo}</p>
            </div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-slate-50 px-3 py-6 md:p-8 lg:mx-6">
