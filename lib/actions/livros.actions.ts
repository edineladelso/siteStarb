...
import { insertLivroSchema, selectLivroSchema } from "@/lib/drizzle/validations";
...
export async function criarLivro(formData: FormData) {
  ...
  const bruto = {
    ...
    status: (formData.get("status") as "rascunho" | "publicado") || "rascunho",
    ...
    numeroPaginas: formData.get("numeroPaginas") ? Number(formData.get("numeroPaginas")) : null,
...
export async function getLivroBySlug(slug: string) {
  const result = await db.select().from(livros).where(eq(livros.slug, slug)).limit(1);
  return result[0] || null;
}

// src/lib/actions/livros.actions.ts

export async function atualizarLivro(id: number, formData: FormData): Promise<ActionResult<Livro>> {
  const titulo = String(formData.get("titulo"));
  ...
  if (!parsed.success) {
    return { success: false, error: "Erro de validação", validationErrors: parsed.error.flatten().fieldErrors };
  }
